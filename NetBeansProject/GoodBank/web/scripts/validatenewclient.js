...
function validateRepeatedDNI(value) {
	...
	$.ajax({
				async: false,
				url: "jQuerySelectClients.jsp",
				type: "POST",
				data: "dni="+value,
				success: function(result) {
					var values=result.split("$");
					var dni=values[0];
				    var clients = values[1].split("#");
				    var clientinfo = [];
				    for (i = 0; i < clients.length; i++)
				    {
					var dataA = clients[i].split(",");
					    clientinfo[i]=dataA;
				    }
				    for (i = 0; i < clientinfo.length; i++)
				    {
				    	//alert((clientinfo[i][1]==(dni.trim())))
						if(clientinfo[i][1]==(dni.trim()))
						{
							returnVal=false;
						}
				    }
...
function checkNewClient(e){
	var validForm=[];
	var dni=$('#clientIdCard').val();
	if(dni.length>0)
	{
	if(validateRepeatedDNI(dni)==false)
		{
		validForm.push("Usuario existente (Ya existe un cliente con este DNI)");
		}
	}
